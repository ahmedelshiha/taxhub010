...
const _api_POST = async (
  ...
  try {
    ...
    // Approve item
    const approval = await approvalsService.approveItem(
      tenantId,
      userId,
      params.id,
      notes
    ...
    if (error instanceof Error) {
      ...
      if (
        error.message === "You are not authorized to approve this item" ||
    ...
    logger.error("Error approving item", { error, approvalId: params.id });
    return NextResponse.json(
